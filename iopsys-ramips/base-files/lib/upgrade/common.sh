...
inteno_image_upgrade() {
	...
	if [ $cfe_sz -gt 0 -a $k_sz -eq 0 -a $ubifs_sz -eq 0 -a $ubi_sz -eq 0 ]; then
	...
	elif [ $cfe_sz -eq 0 -a $k_sz -eq 0 -a $ubifs_sz -gt 0 -a $ubi_sz -eq 0 ] ;then
	    ...
	    if ubiupdatevol /dev/ubi0_$upd_vol --size=$ubifs_sz --skip=$ubifs_ofs $from
	    then
		...
		if cat /proc/cmdline |grep -q "ubi:rootfs_"; then
	    	...
	    	v "KEN Unexpected image file contents, upgrade aborted!"
